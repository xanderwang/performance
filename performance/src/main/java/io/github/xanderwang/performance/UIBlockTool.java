...
class UIBlockTool {
    ...
    private static class DumpBlockInfoRunnable implements Runnable {
        @Override
        public void run() {
            Issue uiIssue = new Issue(Issue.TYPE_UI_BLOCK, "UI BLOCK",
    ...
    private static void hookHandlerDispatchMessage() {
        HookBridge.findAndHookMethod(Handler.class, "dispatchMessage", Message.class, new HandlerDispatchMessageHook());
    }
    ...
    private static void hookDecorViewDispatchKeyEvent() {
        try {
            ...
            HookBridge.findAndHookMethod(decorViewClass, "dispatchKeyEvent", KeyEvent.class,
